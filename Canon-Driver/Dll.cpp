...
STDAPI RegisterFilters( BOOL bRegister )
{
    ...
    if(bRegister)
    {
        hr = AMovieSetupRegisterServer(CLSID_VirtualCam, L"Canon Live View", achFileName, L"Both", L"InprocServer32");
    }
    ...
    if( SUCCEEDED(hr) )
    {
        ...
        if( SUCCEEDED(hr) )
        {
            if(bRegister)
            {
                ...
                hr = fm->RegisterFilter(CLSID_VirtualCam, L"Canon Live View", &pMoniker, &CLSID_VideoInputDeviceCategory, NULL, &rf2);
            }
            else
            {
